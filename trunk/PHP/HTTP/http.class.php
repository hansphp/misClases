...
class H_HTTP {
	...
	{	
			 echo 	"<pre>".$this->REQUEST_HEADERS.CRLF.
			 		"----------------------------".CRLF.
	...
	protected function socket($method, $path = '/', $postdata = '', $headers = '')
	{
		...
		if(!empty($fp)){
			$this->REQUEST_HEADERS	=	"$method $path HTTP/1.1".CRLF.
						'Host: '.$this->HOST.CRLF;
			$this->REQUEST_HEADERS.=		"User-Agent: Mozilla/5.0 (Windows; U; Windows NT 6.1; es-MX; rv:1.9.2.28) Gecko/20120306 Firefox/3.6.28".CRLF;
